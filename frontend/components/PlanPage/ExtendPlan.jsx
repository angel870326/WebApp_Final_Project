...
const handleSubmit = async (event) => {
    ...
    // Call API
    const response = await fetch(`/api/addDonation/${getUser()}`, {
        method: "POST",
        headers: {
    ...
    useEffect(() => {
        if (a_id) {
            async function fetchData() {
                try {
                    const response = await fetch(`/api/getExtendingDonatePlan/${a_id}/${getUser()}`);
                                        ...
                                        <Collapse in={checked}>
                                            <ListItem sx={{ display: 'flex', justifyContent: 'center' }}>
                                                <Typography sx={{ m: 1, border: '1px solid gray', padding: '1rem', display: selectedOption ? 'block' : 'none' }} borderRadius="10px">
                                                    ...
                                                    <br />
                                                    當前認養方案結束日期: {selectedOption ? record.oldEndDate : 'none'}
                                                    <br />
                                                    預計新認養方案開始日期: {selectedOption ? record.newStartDate : 'none'}
                                                </Typography>
                                            </ListItem>
                                        </Collapse>
                                    </List>
                                    <input type="hidden" id="animalId" name="animalId" value={a_id} />
                                    <Button
                                        type="submit"
                                        fullWidth
                                        variant="contained"
