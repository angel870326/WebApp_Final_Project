...
export default function AnimalsInfoPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const response = await fetch('/api/getAnimalInfo/1');
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        // use animalId,memberId = 1,1 just for testing
        const response = await fetch('/api/getAnimalDonateRecord/1/1');
        const jsonData = await response.json();
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        // use animalId,memberId = 1,1 just for testing
        const response = await fetch('/api/getAnimalAdopter/1/1');
        const jsonData = await response.json();
